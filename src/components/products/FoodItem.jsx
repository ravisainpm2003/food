...
// console.log(user);
  ...
  const addToCart = async (action) => {
    ...
    try {
      const response = await axios.post(`${ENDPOINT}add-favorite.php`, data, SecurityHeaders);
          ...
          <div className="list-card bg-white rounded overflow-hidden position-relative shadow-sm">
            <div className="list-card-image">
              ...
              <div className="favourite-heart text-danger position-absolute rounded-circle">
               {isProductInWishlist ? (
                <FavoriteIcon className=' text-danger bg-white rounded rounded-circle' onClick={() => addToCart('delete')}></FavoriteIcon>
         
               ) : (
                <FavoriteBorderIcon className="text-danger bg-white rounded rounded-circle" onClick={() => addToCart()}></FavoriteBorderIcon>
               )}
              </div>
